...
void kernel( int *csr_rows_d, int *csr_cols_d, int *total_sum_d){
  ...
  int i, k, j, start_block_i, stop_block_i, start_block_k, stop_block_k, temp;

  i = blockIdx.x;
  start_block_i = csr_rows_d[i];
  stop_block_i  = csr_rows_d[i + 1] - 1;
  ...
  while (j <= stop_block_i){
    ...
    if( k > blockIdx.x){
      ...
      temp = compare_blocks( csr_cols_d, start_block_i, stop_block_i, start_block_k, stop_block_k);
